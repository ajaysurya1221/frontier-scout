...
| Secret leakage via committed files | Developer accidentally commits `.env`, API key, or secret to git | `.gitignore` covers `.env`, `*.pem`, `*.key`. `.pre-commit-config.yaml` runs `detect-secrets` locally. `bitbucket-pipelines.yml` pull-requests gate runs `detect-secrets` in CI — non-zero finding fails the PR. |
...
| Hidden git-push failures | BB_TOKEN expires; pipeline silently can't push artifacts; audit trail diverges | Pipeline steps distinguish "nothing to commit" (OK) from "push errored" (FAIL). Push failure → step fails → operator notified. |
...
| Lambda Function URL is publicly addressable on the internet | Attacker discovers the URL and sends crafted payloads to trigger Bitbucket pipelines or impersonate Slack | Every request is verified via Slack's HMAC-SHA256 signature scheme (`SLACK_SIGNING_SECRET`) with a 5-minute replay window. `lambda/slack_verify.py` rejects on missing/invalid/stale signatures *before* any dispatch. Slack's docs: <https://api.slack.com/authentication/verifying-requests-from-slack>. |
| Lambda has S3 read + Bitbucket write capabilities | Compromised Lambda runtime could exfiltrate mirror data or trigger unintended Bitbucket pipelines | **Two-role pattern**: (a) bootstrap-time role with `s3:CreateBucket` + bucket-config writes — used *once* during initial setup and then swapped out; (b) runtime role with `s3:GetObject` on the mirror prefix only. `BB_TOKEN` scoped to `Repositories:Write` on a single repo. Slack signature verification gate prevents unauthorized invocations. See "Operator runbook — Lambda role swap" below. |

## Operator runbook

### Secret rotation (every 90 days minimum)

The following secrets live in Bitbucket Repository Variables and must be
...
| `SLACK_BOT_TOKEN` (`xoxb-...`) | api.slack.com/apps → OAuth & Permissions. Bot scopes: `chat:write`, `reactions:write`, `commands`, `chat:write.customize`. Bot must be invited to `#ai-telemetry`. | annually (and on team-member departure) |
...
| `BB_TOKEN` | Bitbucket → Personal settings → API tokens | annually |
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | IAM user scoped to S3 mirror bucket only (`s3:PutObject`, `s3:DeleteObject` on the bucket; nothing else). Used by Bitbucket pipelines for the S3 mirror step. | quarterly |

When rotating: create the new secret first, paste the new value into the
Bitbucket variable, then revoke the old secret. Verify with one manual
`custom: scout` run before relying on the schedule.
...
aws lambda invoke --function-name ai-telemetry-slack \
...
aws iam create-role --role-name ai-telemetry-slack-runtime --assume-role-policy-document file://lambda-trust.json
aws iam put-role-policy --role-name ai-telemetry-slack-runtime --policy-name MirrorRead \
...
aws lambda update-function-configuration \
  ...
  --role arn:aws:iam::ACCOUNT-ID:role/ai-telemetry-slack-runtime
```
...
3. If webhook is rotated, update `SLACK_WEBHOOK_URL` Bitbucket variable, then re-post the dead letters manually.
