...
- **🧪 Lab findings → Mem0 auto-seed** — when an engineer commits findings to `skills-log.md`, a GitHub Actions post-commit hook seeds the finding into Mem0 so `/recall` surfaces it within minutes (today it only feeds the monthly Synthesizer + next Scout). Closes the "we waited a month for the next briefing to know" gap.
